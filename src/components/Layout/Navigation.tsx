...
const Navigation: React.FC<NavigationProps> = ({ mobile = false, onItemClick }) => {
  ...
  const handleSubmenuKeyDown = (e: React.KeyboardEvent, index: number, submenuItems: any[]) => {
    switch (e.key) {
      case 'ArrowDown':
        e.preventDefault();
        ...
        if (nextItem) {
          ...
          (nextLink as HTMLElement | null)?.focus?.();
        }
        break;
      case 'ArrowUp':
        e.preventDefault();
        if (index === 0) {
          ...
          const parentItem = (e.target as HTMLElement).closest('[role="menuitem"]')?.previousElementSibling;
          (parentItem as HTMLElement | null)?.focus?.();
          setActiveDropdown(null);
        } else {
          const prevItem = submenuItems[index - 1];
          const prevLink = document.querySelector(`[href="${prevItem.to}"]`);
          (prevLink as HTMLElement | null)?.focus?.();
        }
        break;
    }
  };
